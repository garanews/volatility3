...
class AArch64(linear.LinearlyMappedLayer):
    ...
    def _translate_entry(self, virtual_offset: int) -> Tuple[int, int, int]:
        ...
        # RPi Zero 2W patch: VA 0xffffff8000000000-0xfffffffbffffffff
        # sono nel linear map vmalloc/task space con traduzione diretta
        # phys = VA - 0xffffff8000000000
        if 0xffffff8000000000 <= virtual_offset <= 0xffffffbfffffffff:
            phys = virtual_offset - 0xffffff8000000000
            if phys < 0x1c000000:  # 448MB RAM max
                page_base = phys & ~0xFFF
                return page_base, 12, self._base_layer
            vollog.debug(f"RPi vmalloc OUT OF RANGE: {hex(virtual_offset)} phys={hex(phys)}")
        ...
        if ttb_selector != self._virtual_addr_space:
            vollog.debug(f"_translate_entry REJECTED: va={hex(virtual_offset)} ttb_sel={ttb_selector} vas={self._virtual_addr_space}")
            raise exceptions.InvalidAddressException(
                layer_name=self.name,
                invalid_address=virtual_offset,
            )
    ...
    def minimum_address(self) -> int:
        base = self._virtual_addr_range[0]
        # RPi Zero 2W: estendi il minimo per coprire vmalloc space 0xffffff80...
        if base >= 0xffffffc000000000:
