...
class LinuxAArch64SubStacker:
    ...
    def stack(
        ...
        if _rpi_override:
            # RPi Zero 2W: va_bits=39 per il page table walk (swapper_pg_dir a 39-bit)
            # Le VA 0xffffff80... sono gestite con bypass diretto in arm.py
            va_bits_candidates = [39]
            page_size_kernel_space_candidates = [4]  # 4K pages
            page_size_kernel_space_candidates = [4]  # 4K pages
        for va_bits in va_bits_candidates:
            ...
            for page_size_kernel_space in page_size_kernel_space_candidates:
                ...
                if _rpi_override:
                    ...
                    self._logger.debug("RPi override: skipping banner verification")
                else:
                    test_banner_equality = self.parent_stacker.verify_translation_by_banner(
                        context=context,
                        layer=layer,
